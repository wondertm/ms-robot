...
module.exports = async (client, message) => {
  ...
  if (message.author.bot){
    //only listen to select bots
      const authorisedBots = message.author ?? message.user;
      var botAllowed = config.allowedBots.includes(authorisedBots.id) ? true : false;
      if(!botAllowed) return;
  }
  ...
  const args = prefix ? message.content.slice(prefix[0].length).trim().split(/ +/g) : message.content.trim().split(/ +/g);
  ...
  // in app.js.
  const cmd = (prefix) ? container.commands.get(command) || container.commands.get(container.aliases.get(command)) : container.commands.get('partialMessage');
  ...
  if (prefix && !cmd.conf.enabled) return;

  if (prefix && level < container.levelCache[cmd.conf.permLevel]) {
...
This command requires level ${container.levelCache[cmd.conf.permLevel]} (${cmd.conf.permLevel})`);
  ...
  if(prefix){
    ...
    try {
      //const partialMessage = container.commands.get('partialMessage');
      await cmd.run(client, message, args, level, command);
    } catch (e) {
      console.error(e);
      message.channel.send({ content: `There was a problem with your request.\n\`\`\`${e.message}\`\`\`` })
        .catch(e => console.error("An error occurred replying on an error", e));
